...
func Load(dsn string, conn int) *DB {
	...
	stmt, err = gdb.Prepare("select title,name,url from (select title,name,url from spider.link_info where name like ? or title like ?  limit 0,?) as A  order by time desc ")
	if err != nil {
		loggo.Error("Prepare mysql fail %v", err)
		return nil
	}
